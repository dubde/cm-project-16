...
//	Rappresentazione 1D: linea
	...
	//	grafica rimane 1 secondo indietro rispetto all'audio, in quanto seziono 
	//	la larghezza del canvas di Fs elementi.
	...
	var x = 0;	
			for(var i = 0; i < Fs; i++) {
				...
				var valore = parseFloat(track.env[i+sampleNow]); 
				valore = (valore * -1) + 1;
				}
				var y = valore * HEIGHT;
				if(i === 0) {
...
//		Rappresentazione barre delle frequenze
		function draw() {
			...
			for(var i = 0; i < chCount; i++) {
				...
				var colorB = Math.trunc(255-valueSample*255);
				canvasCtx.fillStyle = 'rgb('+colorR+','+colorG+','+colorB+')';
